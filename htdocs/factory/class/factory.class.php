...
class Factory extends CommonObject
{
	...
	var $statut;
	var $model_pdf;
	var $datec;		// date de cr�ation de l'OF
	...
	function get_children($fk_product_father, $reqd_stock) {
			#Use fetch_array - neednt specify mysqli_asssoc - default is both this and num

			$sql_get_children_and_qty = "SELECT fk_product_children, qty FROM ".MAIN_DB_PREFIX."llx_product_factory WHERE fk_product_father = ".$fk_product_father;
			$result_get_children_and_qty = $this->db->query($sql_get_children_and_qty);

	    #$result_get_children_and_qty = mysqli_query($conn, $sql_get_children_and_qty);
	    if($result_get_children_and_qty) {
	        $resultArray = [];
	    	  while($row = $this->db->fetch_array($result_get_children_and_qty)) {
	...
	function createof()
	{
		...
		if (! $this->db->query($sql)) {
			...
			// on ajoute � pr�sent le lien (si linked)
...
//var_dump($prod);
	...
	// retourne la liste des produits qui peuvent cr�er un OF
	...
	// r�cup�re la liste des produits d'un of
	function getChildsOF($fk_factory)
	{
		...
		if ($res) {
			...
			while ($rec = $this->db->fetch_array($res)) {
				...
				// remarque, le getchildsarbo �tant r�cursif, on a pas besoin de faire de r�cursivit� ici
